...
**Generated** 2026-09-16 · branch `refactor/zero-waste` · commit `f6df6b8`
...
| | Count | Share |
|---|---:|---:|
| ✅ Executed — proven working | **595** | 38.6% |
| 🟡 Reachable — no fault found | 722 | 46.8% |
| ⏸️ Mutating — not exercised | 166 | 10.8% |
| ⚪ GET, no fixture | 44 | 2.9% |
| ❌ Orphan | **14** | 0.9% |
| **Total** | **1,541** | |

**Nothing is marked "not working."** Across 85 live HTTP routes and 1,036
tests, no function raised, no endpoint returned 5xx, and the backend boots
with zero tracebacks and zero ERROR lines. The only genuine defects found
were 14 orphans and 3 bugs, all listed below and all now fixed.
...
|---|---:|---:|---:|---:|---:|---:|
| HTTP route modules | 861 | 301 | 348 | 166 | 44 | 2 |
| Core (top-level) | 266 | 129 | 130 | 0 | 0 | 6 |
| Knowledge base (`kb/`) | 236 | 98 | 134 | 0 | 0 | 4 |
| CodeGen helpers (`codegen/`) | 65 | 17 | 46 | 0 | 0 | 2 |
...
proven coverage (81%) and no dead code. That is the right shape.
...
| `routes/tools.py` | 176 | 88 | 52 | 21 | 13 | 2 |
| `routes/architecture.py` | 101 | 4 | 77 | 16 | 4 | 0 |
| `routes/srs.py` | 86 | 53 | 22 | 10 | 1 | 0 |
| `routes/kb.py` | 82 | 33 | 28 | 20 | 1 | 0 |
| `routes/living.py` | 59 | 3 | 41 | 11 | 4 | 0 |
| `factory_orchestrator.py` | 53 | 13 | 38 | 0 | 0 | 2 |
...
## The 14 orphans — dead code

Defined, referenced nowhere in the backend.
...
Five of these sit together in `pipeline.py` as a family of stage-context
getters that nothing calls — the live path uses `get_stage_context` /
`require_stage_context` instead. They are the largest single cluster and
the obvious next cleanup.

**Not orphans, despite what a scanner says:** `_suppress_all.__enter__` /
`__exit__` in `fabric/factory_cli.py`. They are invoked by the `with`
protocol at line 559, not by name. A naive dead-code pass flags them.
